 ...
 */
var lightbox=function(h){"use strict";const n={Modal:h.Modal,Carousel:h.Carousel};class i{constructor(t,e={}){this.hash=this.randomHash(),this.settings=Object.assign(Object.assign(Object.assign({},n.Modal.Default),n.Carousel.Default),{interval:!1,target:'[data-toggle="lightbox"]',gallery:"",size:"xl",constrain:!0}),this.settings=Object.assign(Object.assign({},this.settings),e),this.modalOptions=this.setOptionsFromSettings(n.Modal.Default),this.carouselOptions=this.setOptionsFromSettings(n.Carousel.Default),typeof t=="string"&&(this.settings.target=t,t=document.querySelector(this.settings.target)),this.el=t,this.type=t.dataset.type||"",t.dataset.size&&(this.settings.size=t.dataset.size),this.src=this.getSrc(t),this.sources=this.getGalleryItems(),this.createCarousel(),this.createModal()}show(){document.body.appendChild(this.modalElement),this.modal.show()}hide(){this.modal.hide()}setOptionsFromSettings(t){return Object.keys(t).reduce((e,a)=>Object.assign(e,{[a]:this.settings[a]}),{})}getSrc(t){let e=t.dataset.src||t.dataset.remote||t.href||"http://via.placeholder.com/1600x900";if(t.dataset.type==="html")return e;/\:\/\//.test(e)||(e=window.location.origin+e);const a=new URL(e);return(t.dataset.footer||t.dataset.caption)&&a.searchParams.set("caption",t.dataset.footer||t.dataset.caption),a.toString()}getGalleryItems(){let t;if(this.settings.gallery){if(Array.isArray(this.settings.gallery))return this.settings.gallery;t=this.settings.gallery}else this.el.dataset.gallery&&(t=this.el.dataset.gallery);return t?[...new Set(Array.from(document.querySelectorAll(`[data-gallery="${t}"]`),a=>`${a.dataset.type?a.dataset.type:""}${this.getSrc(a)}`))]:[`${this.type?this.type:""}${this.src}`]}getYoutubeId(t){if(!t)return!1;const e=t.match(/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=)([^#&?]*).*/);return e&&e[2].length===11?e[2]:!1}getYoutubeLink(t){const e=this.getYoutubeId(t);if(!e)return!1;const a=t.split("?");let l=a.length>1?"?"+a[1]:"";return`https://www.youtube.com/embed/${e}${l}`}getInstagramEmbed(t){if(/instagram/.test(t))return t+=/\/embed$/.test(t)?"":"/embed",`<iframe src="${t}" class="start-50 translate-middle-x" style="max-width: 500px" frameborder="0" scrolling="no" allowtransparency="true"></iframe>`}isEmbed(t){const a=new RegExp("("+i.allowedEmbedTypes.join("|")+")").test(t),l=/\.(png|jpe?g|gif|svg|webp)/i.test(t)||this.el.dataset.type==="image";return a||!l}createCarousel(){const t=document.createElement("template"),e=i.allowedMediaTypes.join("|"),a=this.sources.map((s,o)=>{s=s.replace(/\/$/,"");const m=new RegExp(`^(${e})`,"i"),$=/^html/.test(s),E=/^image/.test(s);m.test(s)&&(s=s.replace(m,""));const C=this.settings.constrain?"mw-100 mh-100 h-auto w-auto m-auto top-0 end-0 bottom-0 start-0":"h-100 w-100",g=new URLSearchParams(s.split("?")[1]);let p="",r=s;if(g.get("caption")){try{r=new URL(s),r.searchParams.delete("caption"),r=r.toString()}catch{r=s}p=`<div class="carousel-caption d-none d-md-block" style="z-index:2"><p class="bg-secondary rounded">${g.get("caption")}</p></div>`}let c=`<img src="${r}" class="d-block ${C} img-fluid" style="z-index: 1; object-fit: contain;" />`,b="";const S=this.getInstagramEmbed(s),y=this.getYoutubeLink(s);return this.isEmbed(s)&&!E&&(y&&(s=y,b='title="YouTube video player" frameborder="0" allow="accelerometer autoplay clipboard-write encrypted-media gyroscope picture-in-picture"'),c=S||`<img src="${s}" ${b} class="d-block mw-100 mh-100 h-auto w-auto m-auto top-0 end-0 bottom-0 start-0 img-fluid" style="z-index: 1; object-fit: contain;" />`),$&&(c=s),`
			...
			<div id="lightboxCarousel-${this.hash}" class="${u}" data-bs-ride="carousel" data-bs-interval="${this.carouselOptions.interval}">
			    ...
			    ${f}
				${l}
			</div>`;t.innerHTML=v.trim(),this.carouselElement=t.content.firstChild;const w=Object.assign(Object.assign({},this.carouselOptions),{keyboard:!1});this.carousel=new n.Carousel(this.carouselElement,w);const x=this.type&&this.type!=="image"?this.type+this.src:this.src;return this.carousel.to(this.findGalleryItemIndex(this.sources,x)),this.carouselOptions.keyboard===!0&&document.addEventListener("keydown",s=>{if(s.code==="ArrowLeft"){const o=document.getElementById(`#lightboxCarousel-${this.hash}-prev`);return o&&o.click(),!1}if(s.code==="ArrowRight"){const o=document.getElementById(`#lightboxCarousel-${this.hash}-next`);return o&&o.click(),!1}}),this.carousel}findGalleryItemIndex(t,e){let a=0;for(const l of t){if(l.includes(e))return a;a++}return 0}createModal(){const t=document.createElement("template"),e=`
			...
			</div>`;return t.innerHTML=e.trim(),this.modalElement=t.content.firstChild,this.modalElement.querySelector(".modal-body").appendChild(this.carouselElement),this.modalElement.addEventListener("hidden.bs.modal",()=>this.modalElement.remove()),this.modalElement.querySelector("[data-bs-dismiss]").addEventListener("click",()=>this.modal.hide()),this.modal=new n.Modal(this.modalElement,this.modalOptions),this.modal}randomHash(t=8){return Array.from({length:t},()=>Math.floor(Math.random()*36).toString(36)).join("")}}return i.allowedEmbedTypes=["embed","youtube","vimeo","instagram","url"],i.allowedMediaTypes=[...i.allowedEmbedTypes,"image","html"],i.defaultSelector='[data-toggle="lightbox"]',i.initialize=function(d){d.preventDefault(),new i(this).show()},document.querySelectorAll(i.defaultSelector).forEach(d=>d.addEventListener("click",i.initialize)),typeof window<"u"&&window.bootstrap&&(window.bootstrap.Lightbox=i),i}(bootstrap);
