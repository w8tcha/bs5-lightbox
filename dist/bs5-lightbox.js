 ...
 * Copyright 2026 Travis Aaron Wagner (https://github.com/trvswgnr/)
 * Licensed under MIT (https://github.com/trvswgnr/bs5-lightbox/blob/main/LICENSE)
 */
import * as e from "bootstrap";
//#region src/bs5-lightbox.ts
var t = class t {
	hash;
	settings;
	modalOptions;
	...
	setOptionsFromSettings(e) {
		return Object.keys(e).reduce((e, t) => Object.assign(e, { [t]: this.settings[t] }), {});
	}
	getSrc(e) {
		...
		let n = new URL(t);
		return (e.dataset.footer || e.dataset.caption) && n.searchParams.set("caption", e.dataset.footer || e.dataset.caption || ""), n.toString();
	}
	getGalleryItems() {
		let e;
	...
	getInstagramEmbed(e) {
		if (/instagram/.test(e)) return e += /\/embed$/.test(e) ? "" : "/embed", `<iframe src="${e}" class="start-50 translate-middle-x" style="max-width: 500px" frameborder="0" scrolling="no" allowtransparency="true"></iframe>`;
	}
	isEmbed(e) {
	...
	createCarousel() {
		let n = document.createElement("template"), r = t.allowedMediaTypes.join("|"), i = this.sources.map((e, t) => {
			...
			return this.isEmbed(e) && !a && (p && (e = p, d = "title=\"YouTube video player\" frameborder=\"0\" allow=\"accelerometer autoplay clipboard-write encrypted-media gyroscope picture-in-picture\""), u = f || `<img src="${e}" ${d} class="d-block mw-100 mh-100 h-auto w-auto m-auto top-0 end-0 bottom-0 start-0 img-fluid" style="z-index: 1; object-fit: contain;" />`), i && (u = e), `
      ...
      <div id="lightboxCarousel-${this.hash}" class="lightbox-carousel carousel slide" data-bs-ride="carousel" data-bs-interval="${this.carouselOptions.interval}">
		...
		return this.carousel.to(this.findGalleryItemIndex(this.sources, c)), this.carouselOptions.keyboard === !0 && document.addEventListener("keydown", (e) => {
			if (e.code === "ArrowLeft") return document.getElementById(`#lightboxCarousel-${this.hash}-prev`)?.click(), !1;
			if (e.code === "ArrowRight") return document.getElementById(`#lightboxCarousel-${this.hash}-next`)?.click(), !1;
		}), this.carousel;
	}
	...
	createModal() {
      ...
      </div>`.trim(), this.modalElement = t.content.firstChild, this.modalElement.querySelector(".modal-body")?.appendChild(this.carouselElement), this.modalElement.addEventListener("hidden.bs.modal", () => this.modalElement.remove()), this.modalElement.querySelector("[data-bs-dismiss]")?.addEventListener("click", () => this.modal.hide()), this.modal = new e.Modal(this.modalElement, this.modalOptions), this.modal;
	}
	randomHash(e = 8) {
		return Array.from({ length: e }, () => Math.floor(Math.random() * 36).toString(36)).join("");
	}
	static initialize(e) {
		e.preventDefault(), new t(this).show();
	}
};
document.querySelectorAll(t.defaultSelector).forEach((e) => e.addEventListener("click", t.initialize)), typeof window < "u" && window.bootstrap && (window.bootstrap.Lightbox = t);
//#endregion
export { t as default };
